...
-   The last listed in `txhousing` is **`r txhousing$city[nrow(txhousing)]`** .
:::
...
-   The Listing variable is the variable that contains all the total active listing for Texas housing.
:::
...
names(txhousing)
ifelse(sum(names(txhousing) %in% c("inventory")) == 0,
       "Variable Removed", "Variable Not Removed")
```
:::
...
-   This code chunk calculates the mean of the percentage of house sold in each moth and list them in order from highest percentage to lowest percentage.
:::
...
-   In January of 2015, **`r p9_result$city`** had the fewest houses listed for sale with **`r p9_result$listings`** houses listed.
:::
...
```{r}
# Type your code and comments inside the code chunk
p10_result <-txhousing |>
  filter(year == 2012) |>
  group_by(month) |>
  summarize(total_sales = sum(sales, na.rm = TRUE)) |>
  filter(total_sales == max(total_sales))

p10_month_name <- month.name[p10_result$month]
p10_result
```

-   In 2012, the month with the most houses sold in Texas was **`r p10_month_name`** with a total of **`r p10_result$total_sales`** houses sold.
:::
...
# Type your code and comments inside the code chunk
p11_table <- txhousing |>
  filter(city %in% c("Austin", "Dallas"), year %in% c(2000, 2001)) |>
  group_by(city) |>
